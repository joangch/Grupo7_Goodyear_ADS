...
def validar_usuario(usuario: str) -> bool:
    """Valida que el usuario tenga entre 3 y 50 caracteres."""
...
def validar_descripcion(descripcion: str, min_len: int = 10, max_len: int = 1000) -> bool:
    """Valida que la descripción tenga entre min_len y max_len caracteres."""
    if not descripcion:
        return False
    return min_len <= len(descripcion.strip()) <= max_len

def validar_mensaje(mensaje: str, min_len: int = 1, max_len: int = 500) -> bool:
    """Valida que el mensaje tenga entre min_len y max_len caracteres."""
    if not mensaje:
        return False
    return min_len <= len(mensaje.strip()) <= max_len

def validar_imagen(nombre: str, contenido: bytes) -> Optional[str]:
    """Valida extensión y tamaño de imagen. Retorna None si es válida o mensaje de error."""
